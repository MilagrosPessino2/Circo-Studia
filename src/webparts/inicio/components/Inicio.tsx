...
const InicioEstudiante: React.FC<IInicioProps> = ({ context }) => {
  ...
  useEffect(() => {
    const fetchHorarioEnCurso = async () => {
      try {
        ...
        console.log('Materias en curso:', materiasCursando)

        if (materiasCursando.length === 0) {
          console.log('No hay materias en curso para el estudiante')
        }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', minHeight: '100vh' }}>
      ...
      <main style={{ padding: 24 }}>
        ...
        <section>
          ...
          <button style={{ padding: '8px 16px', background: '#bbb', border: 'none', borderRadius: 4 }}>
            Ver coincidencias
          </button>
        </section>
      </main>
