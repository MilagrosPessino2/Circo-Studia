...
interface EstudianteCoincidencia {
    id: number
    nombre: string
    fotoUrl: string
    carreraNombre: string
...
const Coincidencias: React.FC<ICoincidenciasProps> = ({ context }) => {
    ...
    const [coincidenciasMateria, setCoincidenciasMateria] =
        useState<CoincidenciaMateria>({})
    const [filtroCarreraMateria, setFiltroCarreraMateria] = useState<string>(
        'Tecnicatura en desarrollo web'
    )
    const navigate = useNavigate()

    useEffect(() => {
        const cargarColegas = async (): Promise<void> => {
            setLoading(true)
            try {
                ...
                for (const ins of inscripciones) {
                    ...
                    let carreraNombre: string = carrerasMap.get(carreraId) || ''

                    if (!carreraNombre) {
                        try {
                            const carreraItem = await sp.web.lists
                                .getByTitle('Carrera')
                                .items.getById(carreraId)
                                .select('nombre')()
                            ...
                            console.error(
                                `Error al obtener carrera con ID ${carreraId}`,
                                e
                            )
                    ...
                    colegas.push({
                        ...
                        fotoUrl: `/_layouts/15/userphoto.aspx?accountname=${encodeURIComponent(
    ...
    useEffect(() => {
        const cargarCoincidenciasMateria = async (): Promise<void> => {
            try {
                const [estudiantes, estados, inscripciones, carreras] =
                    await Promise.all([
                        sp.web.lists
                            .getByTitle('Estudiante')
                            .items.select(
                ...
                for (const est of estudiantes) {
                    const insc = inscripciones.find(
                        (i) => i.idEstudiante?.ID === est.ID
                    )
                    const carreraNombre =
                        carreraMap.get(insc?.idCarreraId || 0) || 'Desconocido'
                    estMap.set(est.ID, {
                        id: est.ID,
                ...
                for (const estado of estados) {
                    ...
                    if (!estudianteId || !materiaNombre || condicion !== 'C')
                        continue // solo los que están cursando

                    const estudiante: EstudianteCoincidencia | undefined =
                        estMap.get(estudianteId)
                    if (
                        !estudiante ||
                        estudiante.carreraNombre !== filtroCarreraMateria
                    )
                        continue
        ...
        if (modoVista === 'materia') {
            cargarCoincidenciasMateria().catch((err) =>
                console.error('Error cargando coincidencias por materia:', err)
            )
        }
    ...
    const colegasFiltrados = colegas.filter((c) => {
        ...
        const carreraSeleccionadaNormalizado =
            normalizar(carreraSeleccionada).trim()

        const coincideBusqueda =
            nombreUsuarioNormalizado.includes(busquedaNormalizada)
            ...
            <main className={styles.main}>
                <div className={styles.vistaHeader}>
                    <button
                        className={`${styles.tabButton} ${
                            modoVista === 'carrera' ? styles.activo : ''
                        }`}
                    ...
                    <button
                        className={`${styles.tabButton} ${
                            modoVista === 'materia' ? styles.activo : ''
                        }`}
                ...
                {modoVista === 'carrera' && (
                    <div className={styles.controls}>
                        ...
                        >
                            ...
                            <option value='Tecnicatura en desarrollo web'>
                                Tecnicatura en desarrollo web
                            </option>
                ...
                {modoVista === 'carrera' ? (
                    ...
                    <div className={styles.controls}>
                        ...
                        <select
                            value={filtroCarreraMateria}
                            onChange={(e) =>
                                setFiltroCarreraMateria(e.target.value)
                            }
                        >
                            <option value='Tecnicatura en desarrollo web'>
                                Tecnicatura en desarrollo web
                        ...
                        {Object.keys(coincidenciasMateria).length === 0 ? (
                            <p className={styles.noCoincidencias}>
                                No hay coincidencias para esta carrera.
                            </p>
                        ) : (
                            <div className={styles.carreraBloque}>
                                <h3 className={styles.carreraTitulo}>
                                    {filtroCarreraMateria}
                                </h3>

                                {Object.entries(coincidenciasMateria).map(
                                    ([materia, personas], idx) => {
                                        const personasFiltradas =
                                            personas.filter((p) =>
                                                normalizar(p.nombre).includes(
                                        ...
                                        return (
                                            <div
                                                key={idx}
                                                className={styles.bloqueMateria}
                                            >
                                                <strong
                                                    className={
