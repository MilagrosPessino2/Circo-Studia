...
const MisMaterias: React.FC<IMisMateriasProps> = ({ context }) => {
  ...
  const [correlativasInversas, setCorrelativasInversas] = useState<Record<number, number[]>>({}) // clave: correlativaRequerida, valor: materias que la requieren
      ...
      const datos: IMateria[] = estado
        ...
        .map((e) => {
          ...
          const com = comisiones.find(
            (c) => c.codComision === ofertaRelacionada?.codComision?.Id
          )
          ...
          return {
            ...
            estado: estadoFiltro === 'C' ? 'En curso' : estadoFiltro === 'R' ? 'En final' : 'Aprobada',
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <section>
              <select
                ...
                value={estadoFiltro}
                onChange={(e) =>
                  setEstadoFiltro(e.target.value as 'C' | 'A' | 'R')
                }
              >
